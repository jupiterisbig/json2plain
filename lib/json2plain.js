...
JSON2Plain.prototype.__process = function(depth, json) {
  ...
  for (var key in json) {
    ...
    if(Array.isArray(json[key])){
      ...
      for (let index in json[key]) {
        if(typeof json[key][index] === 'object'){
          for (let index1 in json[key][index]) {
            this.output += this.header[depth] + index1 + this.value + json[key][index][index1] + '\n';            
          }
        }
      }
    }
    if (typeof json[key] === 'object') {   
      ...
      if(depth > 1){
        if(isNaN(key)){
            if (Array.isArray(json[key])) {
              this.header[depth] = this.header[depth - 1] + key;
              // console.log(this.header[depth])
            }
            else
              this.header[depth] = this.header[depth - 1] + key + this.list;
        }          
